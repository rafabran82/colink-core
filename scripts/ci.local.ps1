...
# Build slowest table rows (top 10 by seconds, ignoring nulls)
$slowRows = ""
$top = @($sum.durations | Where-Object { # scripts/ci.local.ps1
# Local CI: run pytest, emit JSON summary, append NDJSON metrics, plot, index.html, and zip bundle.
...
.seconds -ne $null } | Sort-Object seconds -Descending | Select-Object -First 10)
foreach ($d in $top) {
