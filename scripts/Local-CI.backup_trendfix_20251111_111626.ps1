...
# ===== Phase-99: Completion Banner =====
$now = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss 'UTC'z")
Write-Host ''
Write-Host "🏁 Local-CI run complete — $now"
Write-Host '----------------------------------------'
...
# ===== Phase-99: CI Trend Chart (runs_log.csv → runs_trend.png) =====
try {
    $csvPath  = ".artifacts\\ci\\runs\\runs_log.csv"
    $plotPath = ".artifacts\\ci\\runs\\runs_trend.png"

    if (Test-Path $csvPath) {
        Write-Host "📈 Generating CI trend chart from $csvPath ..."
...
if len(df) > 0:
    ...
    plt.title('Local-CI Run Trends')
    plt.plot(df['timestamp'], df['artifacts'], marker='o', label='Artifacts')
    plt.plot(df['timestamp'], df['size_mb'], marker='s', label='Total MB')
    plt.plot(df['timestamp'], df['runtime_sec'], marker='^', label='Runtime (sec)')
    ...
    plt.savefig(r'$plotPath', dpi=120)
"@

        python $tmpPy; Remove-Item $tmpPy -ErrorAction SilentlyContinue
        if (Test-Path $plotPath) {
            Write-Host "✅ Trend chart generated: $plotPath"
        } else {
            Write-Warning "⚠️ Chart generation failed (no output file)."
        }
    } else {
