...
# -- Apply robust X-axis labels (avoid DataPoint.AxisLabel) --
$ax = $chart.ChartAreas[0].AxisX
$ax.CustomLabels.Clear()
for ($i = 0; $i -lt $labels.Count; $i++) {
  $cl = New-Object System.Windows.Forms.DataVisualization.Charting.CustomLabel
  $cl.FromPosition = $i - 0.5
  $cl.ToPosition   = $i + 0.5
  $cl.Text         = $labels[$i]
  $null = $ax.CustomLabels.Add($cl)
}
$ax.Interval = 1
...
function Add-Line($name,$vals,$useAxisY2=$false) {
    ...
    for ($i = 0; $i -lt $labels.Count; $i++) {
        # Add point with an X label directly
