...
function Get-PSSummary([string]$folder) {
  ...
  $txLines = 0
  if (Test-Path $tx) {
    $txLines = (Get-Content $tx -ErrorAction SilentlyContinue | Where-Object { [CmdletBinding()]
param(
  [string]$BootstrapDir = ".artifacts/data/bootstrap",
  [string]$SummaryJson  = ".artifacts/ci/bootstrap.summary.json",
  [string]$RunsCsv      = ".artifacts/ci/runs_log.csv",
...
$txLines = 0
if (Test-Path $tx) {
  $lines = Get-Content $tx -ErrorAction SilentlyContinue
  if ($lines) {
    $txLines = ($lines | Where-Object { [CmdletBinding()]
param(
  [string]$BootstrapDir = ".artifacts/data/bootstrap",
  [string]$SummaryJson  = ".artifacts/ci/bootstrap.summary.json",
  [string]$RunsCsv      = ".artifacts/ci/runs_log.csv",
...
function Get-PSSummary([string]$folder) {
  ...
  foreach ($t in $targets) { $present[$t] = Test-Path (Join-Path $folder $t) }
  $tx = Join-Path $folder "tx_log.ndjson"
  $txLines = (Test-Path $tx) ? ((Get-Content $tx -ErrorAction SilentlyContinue | Where-Object { [CmdletBinding()]
param(
  [string]$BootstrapDir = ".artifacts/data/bootstrap",
  [string]$SummaryJson  = ".artifacts/ci/bootstrap.summary.json",
  [string]$RunsCsv      = ".artifacts/ci/runs_log.csv",
...
.Trim() }).Count
  ...
  $ok = ($present.Values -contains $true)
  $json = New-JsonSummary $folder $present $txLines "ps-fallback: presence + tx ndjson line count" $ok
  return $json
}
...
.Trim() -ne '' } | Measure-Object -Line | ForEach-Object { [CmdletBinding()]
param(
  [string]$BootstrapDir = ".artifacts/data/bootstrap",
  [string]$SummaryJson  = ".artifacts/ci/bootstrap.summary.json",
  [string]$RunsCsv      = ".artifacts/ci/runs_log.csv",
