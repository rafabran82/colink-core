...
def main():
    ...
    ap.add_argument("--dir", default="artifacts/charts", help="Charts directory (searches recursively)")
    ...
    d = Path(args.dir)
    pngs = sorted(d.rglob("*.png"))  # recursive
    ...
    if len(pngs) < args.min_files:
        print(f"[metrics][FAIL] need at least {args.min_files} PNGs, got {len(pngs)}"); ok = False
    if total_kb < args.min_total_kb:
        print(f"[metrics][FAIL] need at least {args.min_total_kb} KB total, got {total_kb:.1f} KB"); ok = False
