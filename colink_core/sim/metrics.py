...
def main():
    ...
    ap.add_argument("--max-slip-bps", type=float, default=None, help="Fail if max_slip_bps exceeds this")
    args = ap.parse_args()
    ...
    if len(pngs) < args.min_files:
        print(f"[metrics][FAIL] need >= {args.min_files} PNGs, got {len(pngs)}"); ok = False
    if total_kb < args.min_total_kb:
        print(f"[metrics][FAIL] need >= {args.min_total_kb} KB total, got {total_kb:.1f} KB"); ok = False
    ...
    if args.max_slip_bps is not None:
        ...
        if sp.exists():
            ...
            if mx > float(args.max_slip_bps):
                print(f"[metrics][FAIL] max_slip_bps {mx} > {args.max_slip_bps}"); ok = False
        else:
