...
def main():
    ...
    limiter = TradeLimiter(LimitConfig(
        max_col_in=25_000.0,
        max_dev_bps=2_000.0,
        strikes_window=3,
        cooldown_trades=5,
    ...
    for i, col_in in enumerate(sizes, 1):
        ...
        ok_guard, dev_bps, budget_bps = size_aware_twap_guard(
            poolA, poolB, twap, col_in,
            base_guard_bps=100.0, cushion_bps=150.0, cap_bps=2_000.0
        )

        # Then hard limits / circuit breaker
        ok_limiter, reason = limiter.check_and_record(col_in, dev)

        print(f"[{i}] size={fmt(col_in)} COL | eff={fmt(eff)} TWAP={fmt(tw)} dev={dev:.1f}bps "
