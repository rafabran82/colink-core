...
def try_fill_with_twap_guard(poolA, poolB, col_in: float, twap: TWAP,
                             base_band_bps: float = 100.0, cushion_bps: float = 150.0):
    # Non-mutating quote
    ...
    print(f"Quote: {fmt(col_in)} COL -> {fmt(q.amount_out)} COPX | px={fmt(quote_px)}  TWAP={fmt(ref_px)}  dev={dev:.1f} bps")
    print(f"Guard budget: ≤ {allowed:.1f} bps (base {base_band_bps:.0f} + modeled-impact + cushion {cushion_bps:.0f})")
    ...
    print(f"Exec:  {fmt(col_in)} COL -> {fmt(r_exec.amount_out)} COPX | eff={fmt(r_exec.effective_price)}\n")
    return True
...
def main():
    ...
    print("Applied adverse moves to pools (pre-quote). Mid route price now:", fmt(route_mid_price_copx_per_col(poolA, poolB)), " COPX/COL")
    try_fill_with_twap_guard(poolA, poolB, col_in=5_000.0, twap=tw)
