...
def main():
    ...
    print("Pool XRP⇄COPX: price (COPX per XRP):", fmt(pool_x_copx.y_reserve / pool_x_copx.x_reserve))
    print()

    # Quotes (no mutation)
    q1 = quote_col_to_copx(pool_col_x, pool_x_copx, 5_000.0)
    print("Quote: 5,000 COL -> COPX")
    print("  XRP hop:", fmt(q1.hop1_out), " COPX out:", fmt(q1.amount_out), " eff COPX/COL:", fmt(q1.effective_price))
    print()

    q2 = quote_copx_to_col(pool_col_x, pool_x_copx, 500_000.0)
    print("Quote: 500,000 COPX -> COL")
    print("  XRP hop:", fmt(q2.hop1_out), " COL out:", fmt(q2.amount_out), " eff COL/COPX:", fmt(q2.effective_price))
    print()
    ...
    print("  XRP hop:", fmt(r_exec.hop1_out), " COPX out:", fmt(r_exec.amount_out), " eff COPX/COL:", fmt(r_exec.effective_price))
    print("  New prices:",
          "COL/XRP =", fmt(poolA.y_reserve / poolA.x_reserve),
          "COPX/XRP =", fmt(poolB.y_reserve / poolB.x_reserve))
    print()
