...
# Quiet import of xrpay (mute name-check noise locally)
...
if (-not $env:XRPL_RPC) {
  $env:XRPL_RPC = "https://s.altnet.rippletest.net:51234"
}
...
if (-not $p1.engine_result -or $p1.engine_result -ne "tesSUCCESS") {
  throw "STEP1 not tesSUCCESS: $(($p1 | ConvertTo-Json -Depth 6))"
}
if (-not $p1.hash) { throw "STEP1 missing hash" }

# STEP 2 — issuer pays COPX
$step2Py = Join-Path $PSScriptRoot "otc_step2_issuer_pays_copx.py"
$p2      = Invoke-PyJson $step2Py
if (-not $p2.engine_result -or $p2.engine_result -ne "tesSUCCESS") {
  throw "STEP2 not tesSUCCESS: $(($p2 | ConvertTo-Json -Depth 6))"
}
if (-not $p2.hash) { throw "STEP2 missing hash" }
...
[int64]$estGross = 0
try {
  $estGross = [int64]([math]::Round(([decimal]$QtyCOPX * [decimal]$PriceXRP) * 1000000))
} catch { }

# Build rich receipt (schema=1.0)
...
# Save JSON (UTF-8). Filename with timestamp.
